function [qrs_amp_raw,qrs_i_raw,delay]=Qrs(ecg,fs,gr)
%None
%% function [qrs_amp_raw,qrs_i_raw,delay]=qrs(ecg,fs)
...
for i = 1 : length(pks)
  ...
  %%Find the mean RR of entire segment
        rr_cust_mean = mean(diff(qrs_i));
...
line(repmat(qrs_i_raw,[2 1]),repmat([min(ecg-mean(ecg))/2; max(ecg-mean(ecg))/2],size(qrs_i_raw)),'LineWidth',2.5,'LineStyle','-.','Color','b');
linkaxes(az,'x');
zoom on;
end
end
